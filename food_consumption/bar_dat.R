bar_dat <- function(x, selected_country, sorting, axis_setting){
  ...
  titl <- paste0('<span style="color:#E69F00"> CO2 emmissions </span>and ',
                 '<span style="color:#009E73">Consumption </span> by food category for ',
  ...
  plot <-  ggplot(by_country, aes(x = food_category, y = kg, fill = type))+
    ...
    scale_fill_manual(values = c("#E69F00", "#009E73"))+ # set colours to match colour scheme
